#!/usr/bin/env bash
#
# Assembles build/blog-voyeur/ and build/blog-voyeur.zip from the tracked files
# at HEAD (via `git archive`), minus dev-only paths.
#
...
# Remove tracked dev-only paths that must not ship (this release tooling, etc.).
dev_paths=(
	release.config.json
	scripts
	package.json
...
# Drop top-level dotfiles (.github, .gitignore, ...).
